...
public class HomeController implements CommunityConstant {
    ...
    public String getIndexPage(Model model, Page page,
                               @RequestParam(name="orderMode",defaultValue = "0") int orderMode){ //通过model携带数据给模板  页面会传入分页有关的条件 通过page封装
        //方法调用前 springMVC会自动实例化Model和Page 并将Page注入Model
        //所以在thymeleaf中可以直接访问Page对象中的数据 不需要再次model.addattribute.....
        page.setRows(discussPostService.findDiscussPostRows(0));
        page.setPath("/index?orderMode=" + orderMode);
        List<DiscussPost> list = discussPostService.findDiscussPosts(0,page.getOffset(),page.getLimit(),orderMode);//返回的对象中包含的是uerId不是用户名需要再处理一下
